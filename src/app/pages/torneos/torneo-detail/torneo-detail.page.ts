...
import {
  ...
  IonFab, IonFabButton, IonAlert, IonActionSheet, IonMenuButton, ActionSheetController,
  AlertController
...
@Component({
  ...
  imports: [
    ...
    IonRefresher, IonRefresherContent, IonSkeletonText, IonToast, IonAvatar,
    IonFab, IonFabButton, IonAlert, IonActionSheet, IonMenuButton
  ]
})
export class TorneoDetailPage implements OnInit {
  ...
  // ✅ TABS: valor por defecto 'resumen', puede ser sobreescrito por query param
  ...
  // Tabs válidos que acepta la vista
  private readonly TABS_VALIDOS = ['resumen', 'equipos', 'posiciones', 'partidos'];

  constructor() {
    addIcons({
      ellipsisVertical, refreshOutline, trophyOutline, calendarOutline,
      peopleOutline, personOutline, cashOutline, gitBranchOutline, playOutline,
      arrowForwardOutline, closeCircleOutline, statsChartOutline, timeOutline,
      radioButtonOn, footballOutline, chevronForwardOutline, addOutline,
      shieldOutline, medalOutline, locationOutline, gridOutline, createOutline,
      listOutline, checkmarkCircleOutline, alertCircleOutline
    });
  }
  ...
  ngOnInit() {
    ...
    // ✅ NUEVO: leer el tab desde query params (?tab=partidos)
    const tabParam = this.route.snapshot.queryParamMap.get('tab');
    if (tabParam && this.TABS_VALIDOS.includes(tabParam)) {
      this.segmentValue = tabParam;
    }

    // ✅ NUEVO: también leer filtro de partidos si viene por query param
    const filtroParam = this.route.snapshot.queryParamMap.get('filtro');
    if (filtroParam && ['todos', 'programados', 'finalizados'].includes(filtroParam)) {
      this.filtroPartidos = filtroParam;
    }

    if (id) {
      this.torneoId = +id;
      this.storageService.setItem('torneoActivoId', id);
      this.cargarDatos();
    } else {
  ...
  cargarPartidos() {
    this.partidoService.listarPartidosPorTorneo(this.torneoId).subscribe({
      next: (data) => {
        ...
        this.resolverCampeonSiFalta();
      },
      error: (error) => { console.error('Error al cargar partidos:', error); }
    });
  }

  resolverCampeonSiFalta() {
    if (this.torneo && this.torneo.estado === 'FINALIZADO' && !this.torneo.campeonNombre) {
      // Buscar la final
      const finalMatch = this.partidos.find(p => p.fase === 'FINAL' && p.estado === 'FINALIZADO');
      if (finalMatch) {
        if (finalMatch.ganadorNombre) {
          this.torneo.campeonNombre = finalMatch.ganadorNombre;
          this.torneo.campeonId = finalMatch.ganadorId;
          const esLocal = finalMatch.ganadorId === finalMatch.equipoLocalId;
          this.torneo.subcampeonNombre = esLocal ? finalMatch.equipoVisitanteNombre : finalMatch.equipoLocalNombre;
        } else if (finalMatch.golesLocal != null && finalMatch.golesVisitante != null) {
          let esLocal = false;
          if (finalMatch.golesLocal > finalMatch.golesVisitante) {
            esLocal = true;
          } else if (finalMatch.golesLocal < finalMatch.golesVisitante) {
            esLocal = false;
          } else if (finalMatch.penalesLocal != null && finalMatch.penalesVisitante != null) {
            esLocal = finalMatch.penalesLocal > finalMatch.penalesVisitante;
          }
          this.torneo.campeonNombre = esLocal ? finalMatch.equipoLocalNombre : finalMatch.equipoVisitanteNombre;
          this.torneo.subcampeonNombre = esLocal ? finalMatch.equipoVisitanteNombre : finalMatch.equipoLocalNombre;
        }
      }
    }
  ...
  cargarTablaPosiciones() {
    this.equipoService.obtenerTablaPosiciones(this.torneoId).subscribe({
      ...
      error: (error) => { console.error('Error al cargar tabla de posiciones:', error); }
    });
  }
  ...
  separarPartidos() {
    this.partidosEnVivo = this.partidos.filter(p => p.estado === 'EN_CURSO');
  ...
  finalizarTorneo() {
    // LLamar al proceso normal de avanzarFase para que el backend calcule el Campeón automáticamente
    this.avanzarFase();
  }
  ...
  // ✅ NUEVO: navegar a pestaña de partidos actualizando la URL (para bookmarking)
  verTodosPartidos() {
  ...
  verPartidosFinalizados() {
    ...
    this.router.navigate([], {
      relativeTo: this.route,
      queryParams: { tab: 'partidos', filtro: 'finalizados' },
      queryParamsHandling: 'merge',
      replaceUrl: true
    });
  }
  ...
  verPartidosProgramados() {
    ...
    this.router.navigate([], {
      relativeTo: this.route,
      queryParams: { tab: 'partidos', filtro: 'programados' },
      queryParamsHandling: 'merge',
      replaceUrl: true
    });
  }
  ...
  async mostrarOpcionesTorneo() {
    ...
    if (this.torneo?.estado === 'INSCRIPCIONES_ABIERTAS') {
      buttons.push({ text: 'Generar Fixture', icon: 'git-branch-outline', handler: () => { this.generarFixture(); } });
    }
    if (this.torneo?.estado === 'INSCRIPCIONES_CERRADAS') {
      buttons.push({ text: 'Iniciar Torneo', icon: 'play-outline', handler: () => { this.iniciarTorneo(); } });
    }
    if (this.puedeAvanzarFase()) {
      buttons.push({ text: 'Avanzar Fase', icon: 'arrow-forward-outline', handler: () => { this.avanzarFase(); } });
    }
    if (this.torneo?.estado === 'FINAL') {
      buttons.push({ text: 'Finalizar Torneo', icon: 'trophy-outline', handler: () => { this.finalizarTorneo(); } });
    }
    if (this.puedeCancelar()) {
      buttons.push({ text: 'Cancelar Torneo', icon: 'close-circle-outline', role: 'destructive', handler: () => { this.confirmarCancelar(); } });
    }
  ...
  async mostrarAccionesRapidas() {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [
        { text: 'Agregar Equipo', icon: 'people-outline', handler: () => { this.agregarEquipo(); } },
        { text: 'Ver Partidos', icon: 'football-outline', handler: () => { this.verTodosPartidos(); } },
        { text: 'Ver Posiciones', icon: 'trophy-outline', handler: () => { this.segmentValue = 'posiciones'; } },
