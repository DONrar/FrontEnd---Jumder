...
import {
  ...
  IonInput, IonAlert, IonAvatar, IonSpinner, IonText, IonMenuButton, IonDatetime
...
import {
  ...
  swapHorizontalOutline, ellipseOutline, ribbonOutline, saveOutline,
  settingsOutline, optionsOutline, create, chevronDownCircleOutline
} from 'ionicons/icons';
...
@Component({
  ...
  imports: [
    ...
    IonList, IonItem, IonChip, IonGrid, IonRow, IonCol, IonRefresher, IonRefresherContent,
    IonSkeletonText, IonToast, IonBadge, IonModal,
    IonSelect, IonSelectOption, IonInput, IonAlert, IonAvatar, IonSpinner, IonText, IonMenuButton, IonDatetime
  ]
})
export class PartidoDetailPage implements OnInit {
  ...
  abrirModalProgramacion() {
    if (!this.partido) return;
    this.editFechaHora = this.partido.fechaHora;
    this.editObservaciones = this.partido.observaciones || '';
    this.showProgramacionModal = true;
  }

  cerrarModalProgramacion() {
    this.showProgramacionModal = false;
  }

  guardarProgramacion() {
    if (!this.partido || !this.editFechaHora) return;

    this.actionLoading = true;
    const partidoActualizado: PartidoDTO = {
      ...this.partido,
      fechaHora: this.editFechaHora,
      observaciones: this.editObservaciones
    };

    this.partidoService.actualizarPartido(this.partidoId, partidoActualizado).subscribe({
      next: () => {
        this.mostrarToast('¡Programación actualizada con éxito!', 'success');
        this.showProgramacionModal = false;
        this.actionLoading = false;
        this.cargarDatos();
      },
      error: (error) => {
        ...
        this.mostrarToast('Error al actualizar la programación', 'danger');
        this.actionLoading = false;
      }
    });
  }
